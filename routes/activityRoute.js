...
const StockAdded = require("../models/StockAdded.js");
...
router.post(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    const stockAdded = new StockAdded({
      ...
      quantity: 0 - Number(req.body.quantityOfProduct),
      comment: "used for treatment",
      additionType: "deduction",
      addedDate: new Date(req.body.treatmentDate),
      treatmentID:savedTreatment._id
    });
    await stockAdded.save()
...
router.get(
  "/treatment",
  expressAsyncHandler(async (req, res) => {
    const treatments = await Treatment.find().populate("field").sort({createdAt:-1});
    res.send(treatments);
  })
);
router.get(
...
router.put(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    const stockAdded = await  StockAdded.findOne({treatmentID:treatment._id})
    console.log(stockAdded)
    if(stockAdded){
      stockAdded.item = req.body.productID || stockAdded.productID
      stockAdded.addedDate = req.body.treatmentDate
      ? new Date(req.body.treatmentDate)
      : treatment.treatmentDate;
       stockAdded.quantity = 0 - Number(req.body.quantityOfProduct) || stockAdded.quantity
      await stockAdded.save()
    }
...
router.put('/task/paid/:id',isAuth,expressAsyncHandler(async(req,res)=>{
  ...
  res.send({task:savedTask,message:"Task updated successfully"})
